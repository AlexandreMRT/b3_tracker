...
from models import Asset, Portfolio, Position, Quote, Transaction, TransactionType
...
def get_user_portfolios(db: Session, user_id: str) -> List[Portfolio]:
    ...
    return (
        ...
        .order_by(desc(Portfolio.is_default), Portfolio.created_at)
        .all()
    )
...
def create_portfolio(
    db: Session, user_id: str, name: str, description: str = None, is_default: bool = False
) -> Portfolio:
...
def update_portfolio(
    db: Session, portfolio_id: int, user_id: str, name: str = None, description: str = None, is_default: bool = None
) -> Optional[Portfolio]:
...
def get_position_by_ticker(db: Session, portfolio_id: int, ticker: str) -> Optional[Position]:
    ...
    return db.query(Position).filter(Position.portfolio_id == portfolio_id, Position.ticker == ticker.upper()).first()
...
def get_ticker_transactions(db: Session, portfolio_id: int, ticker: str) -> List[Transaction]:
    ...
    return (
        db.query(Transaction)
        .filter(Transaction.portfolio_id == portfolio_id, Transaction.ticker == ticker.upper())
        .order_by(desc(Transaction.transaction_date))
        .all()
    )
...
def delete_transaction(db: Session, transaction_id: int, portfolio_id: int) -> bool:
    ...
    transaction = (
        db.query(Transaction).filter(Transaction.id == transaction_id, Transaction.portfolio_id == portfolio_id).first()
    )
...
def calculate_portfolio_performance(db: Session, portfolio_id: int) -> Dict:
    ...
    # Get dividend income
    dividends = (
        db.query(func.sum(Transaction.total_brl))
        .filter(Transaction.portfolio_id == portfolio_id, Transaction.transaction_type == TransactionType.DIVIDEND)
        .scalar()
        or 0.0
    )
